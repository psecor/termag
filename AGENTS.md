...
- **Workstream** — a named parallel line of work inside a project, backed by a git worktree. Every project has a `main` workstream auto-created on project creation; additional workstreams are managed through `backend/src/routes/workstreams.ts` (orchestrator) and the `git-worktree-add` / `git-worktree-remove` RPCs in `agent/agent.js`. Workstreams live at `<projectDir>/.worktrees/<name>/`. Tmux session naming threads the workstream id through (`<user>-<project>-<workstream>-<role>`, collapsing the workstream segment to keep historical names when it's `main`). Branching off mirrors the project's main-workstream `Workflow` rows onto the new workstream (one per workstream, since the legacy `(projectId, type)` unique was dropped) and launches its tmux sessions so the terminal pane can attach immediately. The UI surfaces a workstream sub-list under projects with >1 workstream and a "Branch off…" overflow action; single-workstream projects render exactly as before.
...
- `20260601011055_drop_workflow_project_unique` — drops the legacy `(projectId, type)` unique on `workflows` so a project can carry one workflow per workstream; `(workstreamId, type)` remains as the per-workstream guarantee.
