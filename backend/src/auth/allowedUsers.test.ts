...
describe('resolveUnixUsername', () => {
  const config = parseAllowedUsers(
    'Admin@example.com:root, @launchdarkly.com, @acme.com:deploy'
  );
  ...
  it('prefers an exact match over a domain wildcard', () => {
    const c = parseAllowedUsers('jane@launchdarkly.com:special, @launchdarkly.com');
    expect(resolveUnixUsername(c, 'jane@launchdarkly.com')).toBe('special');
  });

  it('derives the unix user from the local part for a bare domain wildcard', () => {
    expect(resolveUnixUsername(config, 'vkorolik@launchdarkly.com')).toBe('vkorolik');
  });

  it('lowercases the derived username and matches the domain case-insensitively', () => {
    expect(resolveUnixUsername(config, 'Vadim@LAUNCHDARKLY.com')).toBe('vadim');
  });

  it('strips plus-addressing aliases from the derived username', () => {
    expect(resolveUnixUsername(config, 'jane+work@launchdarkly.com')).toBe('jane');
  });
