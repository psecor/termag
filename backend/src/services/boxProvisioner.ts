 ...
 * terraform/box/cloudinit.sh.tftpl — the baked termag-agent.service runs
 * `agent.js ~termag/src/termag/agent/agent.config.json`, so the config MUST
 * land at that exact path (NOT ~/.config/termag — see packer setup.sh note).
 *
...
export function buildCloudInit(v: CloudInitVars): string {
  ...
  const agentConfig = JSON.stringify({
    ...
    token: v.agentToken,
    path_remap: { [`/home/${v.remoteUnixUser}`]: '/home/termag' },
  }, null, 2);

  // Single-quoted heredoc so nothing in the JSON is shell-expanded. Git
  // identity values are passed via env to avoid quoting pitfalls.
  return `#!/bin/bash
# termag box first-boot wiring (generated by boxProvisioner.ts).
# Ported from terraform/box/cloudinit.sh.tftpl.
#
# SECURITY: the only secret below is the per-box agent bearer token. It has no
# AWS authority — the instance role + permissions boundary is the real ceiling.
set -euo pipefail
...
# The baked systemd unit reads ~termag/src/termag/agent/agent.config.json.
...
sudo -u termag -H git config --global user.email "$TERMAG_GIT_EMAIL"
sudo -u termag -H git config --global user.name  "$TERMAG_GIT_NAME"

# ── Enable + start the agent ──────────────────────────────────────────────
# termag has linger enabled in the AMI, so the user-manager already exists.
