...
6. **Commits dated before the public-scrub fork point are already on origin, even though message-search can't find them.** `86cb06e chore: scrub LD-specific and personal-host references` (2026-05-31) is the fork point. Origin's `89d0e28 termag — initial commit` is a *squashed* snapshot of all labs history up to that point — individual commit messages from before 2026-05-31 don't appear in `git log origin/main`. So `git log labs/main --not origin/main --grep="..."` will list pre-scrub commits as "not on origin", but the code is already there in evolved form. Before cherry-picking any commit dated < 2026-05-31, **verify the feature isn't already on origin** by `grep`-ing for its function/symbol name in the corresponding file on `origin/main`. (Spot-checked once during the June 2026 batch: dev-login, ws heartbeat, and agentRuntime probe were all already on origin despite their messages being missing.)
